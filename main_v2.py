...
# Jian, X., Xia, Y., Duth̩, G., Bacsa, K., Liu, W., & Chatzi, E. (2024). Using Graph Neural Networks and Frequency Domain Data for Automated Operational Modal Analysis of Populations of Structures. arXiv preprint arXiv:2407.06492.
...
if model_name == "GraphSAGE":
    class Model(nn.Module):  # original model
        def __init__(self, encoder_hid_dim, encoder_out_dim, GNN_hid_dim, GNN_out_dim, decoder_hid_dim):
            ...
            self.phi_estimator = MLP(GNN_out_dim, decoder_hid_dim, modeN) # Define a MLP to decode node representations into mode shapes   
            self.freq_estimator = MLP(GNN_out_dim, encoder_hid_dim, modeN)
            self.damping_estimator = MLP(GNN_out_dim, encoder_hid_dim, modeN)
        def forward(self, g):
            h_node = self.node_encoder(g.ndata['acc_Y'])
            h_GNN = self.GNN(g, h_node)
            ms = self.phi_estimator(h_GNN)
            g.ndata['h'] = h_GNN
    ...
    class Model(nn.Module):  # original model
        def __init__(self, encoder_hid_dim, encoder_out_dim, GNN_hid_dim, GNN_out_dim, decoder_hid_dim):
            super().__init__()
            self.node_encoder = MLP(input_dim, encoder_hid_dim, encoder_out_dim) # Define a MLP to encode the nodal features into node representations
            self.GNN = GCN(encoder_out_dim, GNN_hid_dim, GNN_out_dim) # Message passing with GCN
            self.phi_estimator = MLP(GNN_out_dim, decoder_hid_dim, modeN) # Define a MLP to decode node representations into mode shapes   
            self.freq_estimator = MLP(GNN_out_dim, encoder_hid_dim, modeN)
            self.damping_estimator = MLP(GNN_out_dim, encoder_hid_dim, modeN)
        def forward(self, g):
    ...
    class Model(nn.Module):  # original model
        def __init__(self, encoder_hid_dim, encoder_out_dim, GNN_hid_dim, GNN_out_dim, decoder_hid_dim):
            super().__init__()
            self.node_encoder = MLP(input_dim, encoder_hid_dim, encoder_out_dim) # Define a MLP to encode the nodal features into node representations
            self.GNN = GAT(encoder_out_dim, GNN_hid_dim, GNN_out_dim) # Message passing with GAT
            self.phi_estimator = MLP(GNN_out_dim, decoder_hid_dim, modeN) # Define a MLP to decode node representations into mode shapes   
            self.freq_estimator = MLP(GNN_out_dim, encoder_hid_dim, modeN)
            self.damping_estimator = MLP(GNN_out_dim, encoder_hid_dim, modeN)
        def forward(self, g):
            h_node = self.node_encoder(g.ndata['acc_Y'])
            h_GNN = self.GNN(g, h_node).squeeze()
            ms = self.phi_estimator(h_GNN)
            g.ndata['h'] = h_GNN
            h_graph = dgl.readout_nodes(g, 'h', op='mean')
            f = self.freq_estimator(h_graph)
        ...
        class Model(nn.Module):  # model without encoder
            ...
            def forward(self, g):
                ...
                zeta = self.damping_estimator(h_graph)
                return f, zeta, ms # return frequencies and mode shapes
elif model_name == "No_GNN":
        class Model(nn.Module):  # model using MLP to identify mode shapes
            def __init__(self, encoder_hid_dim, encoder_out_dim, GNN_hid_dim, GNN_out_dim, decoder_hid_dim):
                super().__init__()
                self.node_encoder = MLP(input_dim, encoder_hid_dim, encoder_out_dim) # Define a MLP to encode the nodal features into node representations
                self.phi_estimator = MLP(encoder_out_dim, decoder_hid_dim, modeN) # Define a MLP to decode node representations into mode shapes   
...
if use_wandb == 1:
    wandb.init(
        ...
        project="ModalGNN_FrequencyDomain",
...
for epoch in range(n_epoch):
    ...
    loss_meter_valid_phi.append(epoch_loss_valid_phi)
    loss_meter_valid_freq.append(epoch_loss_valid_freq)
    loss_meter_valid_zeta.append(epoch_loss_valid_zeta)
    loss_meter_valid.append(epoch_loss_valid)
    ...
    for graph_train, freq_train, zeta_train in dataloader_train:
        ...
        # loss about phi
        loss_train_phi = loss_mse(phi_pred_train.squeeze(), graph_train.ndata['phi_Y'])
        # loss about zeta
        loss_train_zeta = loss_mse(zeta_pred_train/zeta_train, torch.ones(zeta_train.shape).to(device))
        # loss about freq
    ...
    loss_meter_train_phi.append(epoch_loss_train_phi)
    loss_meter_train_freq.append(epoch_loss_train_freq)
    loss_meter_train_zeta.append(epoch_loss_train_zeta)
    loss_meter_train.append(epoch_loss_train)
    ...
    if use_wandb == 1:
        wandb.log({"loss_train": loss_train_phi + loss_train_zeta + loss_train_freq,
...
freq_pred = np.zeros([len(zeta), modeN])
zeta_pred = np.zeros([len(zeta), modeN])
phi_pred = []

start_time = time.time()
# modal identification
for caseN in range(len(zeta)):
    if caseN % 100 == 0:
        print(caseN)   
    test_data = Dataset(graph_ids = [caseN])[0]
    graph_test = test_data[0].cpu()           
...
for k in range(3):
    if k == 0:
        ax[k].plot(MAC_phi[:, 3], label='model')  # the fourth mode
